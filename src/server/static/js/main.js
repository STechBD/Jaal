...
const fetchNews = async () => {
	...
	try {
		const response = await fetch('https://news.ulkaa.com/wp-json/wp/v2/posts?_fields=title,link,date&per_page=12');
		const data = await response.json();
		cacheData('news', data);
		updateNews(data);
	} catch (error) {
	...
	newsList.innerHTML = articles.map(article => `
		<li class="py-2 border-b">
			...
			<p class="text-sm text-gray-500">
				${new Date(article.date).toLocaleString('en-US', { dateStyle: 'long', timeStyle: 'short' })}
			</p>
		</li>
	`).join('');
};
